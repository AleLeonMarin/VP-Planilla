...
exports.NomineeService = void 0;
const ClockLogsService_1 = require("../service/ClockLogsService");
const client_1 = require("@prisma/client");
const DeductionsService_1 = require("./DeductionsService");
const EmployeeService_1 = require("./EmployeeService");
const PositionService_1 = require("./PositionService");
const VacationService_1 = require("./VacationService");
const PayrollUtils = __importStar(require("../utils/payrollUtils"));
...
class NomineeService {
    ...
    async calculateNominee() {
        ...
        for (const log of logs) {
            const employeeDeductions = await this.getEmployeeDeductions(log.employee_id);
            ...
            for (const employeeDeduction of employeeDeductions) {
                const deduction = await DeductionsService_1.DeductionsService.getDeductionById(employeeDeduction.deduction_id);
                if (!deduction) {
                ...
                console.log(`Total de deducciones para el empleado ${log.employee_id}: $${sum.toFixed(2)}`);
            }
        }
    }
    /**
    ...
    async calculatePayrollForPeriod(startDate, endDate) {
        ...
        try {
            ...
            let employees = await EmployeeService_1.EmployeeService.getActiveEmployeesForPeriod(startDate, endDate);
            // If none found, fallback to all employees to aid troubleshooting and avoid returning an empty result silently
            if (employees.length === 0) {
                const allEmployees = await EmployeeService_1.EmployeeService.getAllEmployees();
                result.summary.messages.push(`No se encontraron empleados activos para el periodo. Total en sistema: ${allEmployees.length}.` +
            ...
            for (const employee of employees) {
                try {
                    const employeePayroll = await this.calculateEmployeePayroll(employee, startDate, endDate);
                    ...
                    result.summary.messages.push(`Error procesando empleado ${employee.name} (ID: ${employee.id}): ${error instanceof Error ? error.message : 'Error desconocido'}`);
            ...
            result.summary.messages.push(`Error general en el cálculo de nómina: ${error instanceof Error ? error.message : 'Error desconocido'}`);
        }
        return result;
    }
    /**
     ...
     */
    async calculateEmployeePayroll(employee, startDate, endDate) {
        ...
        try {
            ...
            if (employee.position_id) {
                ...
                employeePayroll.generalMessages.push('Advertencia: Empleado sin puesto asignado. Usando salario base de 0.');
            }
            // Get clock logs for the period
            const clockLogsService = new ClockLogsService_1.ClockLogsService();
            ...
            const employeeVacations = vacations.filter(vacation => vacation.employee_id === employee.id &&
                vacation.paid &&
                this.dateRangesOverlap(vacation.start_date, vacation.end_date, startDate, endDate));
            // Process daily work
            const dailyWork = this.processDailyWork(employeeClockLogs, employeeVacations, startDate, endDate, employee.name);
            ...
            employeePayroll.grossSalary = PayrollUtils.roundToMoney(totalHoursWorked * employeePayroll.baseHourlySalary + employeePayroll.bonuses);
            // Calculate deductions
            const deductionsData = await this.calculateDeductions(employee.id, employeePayroll.grossSalary);
            ...
            employeePayroll.netSalary = PayrollUtils.calculateNetSalary(employeePayroll.grossSalary, employeePayroll.totalDeductions);
            if (originalNetSalary < 0) {
                employeePayroll.generalMessages.push('Advertencia: Salario neto calculado como negativo, se estableció en 0.');
            }
        }
        catch (error) {
            console.error(`Error calculating payroll for employee ${employee.id}:`, error);
            employeePayroll.generalMessages.push(`Error en el cálculo: ${error instanceof Error ? error.message : 'Error desconocido'}`);
        }
     ...
     */
    processDailyWork(clockLogs, vacations, startDate, endDate, employeeName) {
        const days = [];
        ...
        while (currentDate <= endDate) {
            ...
            if (isVacationDay) {
                ...
                if (dayClockLogs.length > 0) {
                    dayWork.messages.push(`Advertencia para ${employeeName} el ${dateStr}: día de vacaciones con marcajes detectados. Se prioriza vacaciones (8h).`);
                }
            }
            else {
     ...
     */
    calculateDailyHours(dayClockLogs, dateStr, employeeName) {
        const messages = [];
        ...
        if (PayrollUtils.hasOverlappingPairs(validation.pairs)) {
            messages.push(`Inconsistencia de marcaje para ${employeeName} el ${dateStr}: periodos de trabajo superpuestos detectados. Revisar.`);
            return { hours: 0, messages, hasInconsistency: true };
        }
     ...
     */
    async calculateBonuses(employeeId, startDate, endDate) {
        try {
     ...
     */
    async calculateDeductions(employeeId, grossSalary) {
        const breakdown = [];
        let total = 0;
        try {
            ...
            for (const empDeduction of employeeDeductions) {
                try {
                    ...
                    let name = empDeduction.deduction_name;
                    let fixed = empDeduction.fixed_amount;
                    let percent = empDeduction.percentage;
                    // Fallback: fetch full definition if missing
                    if ((fixed == null || percent == null) && !name) {
                        const deduction = await DeductionsService_1.DeductionsService.getDeductionById(empDeduction.deduction_id);
                        if (!deduction) {
                            breakdown.push({
...
exports.NomineeService = NomineeService;
