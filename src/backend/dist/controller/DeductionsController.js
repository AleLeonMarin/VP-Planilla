...
const DeductionsService_1 = require("../service/DeductionsService");
class DeductionsController {
    ...
    static async createDeduction(req, res) {
        try {
            const deduction = await DeductionsService_1.DeductionsService.createDeduction(req.body);
    ...
    static async getAllDeductions(req, res) {
        try {
            const deductions = await DeductionsService_1.DeductionsService.getAllDeductions();
    ...
    static async updateDeduction(req, res) {
        ...
        try {
            const updatedDeduction = await DeductionsService_1.DeductionsService.updateDeduction(deductionId, deductionData);
            if (!updatedDeduction) {
    ...
    static async deleteDeduction(req, res) {
        const deductionId = parseInt(req.params.id, 10);
        try {
            const deleted = await DeductionsService_1.DeductionsService.deleteDeduction(deductionId);
            if (!deleted) {
                return res.status(404).json({ error: "Deduction not found" });
            }
            return res
