...
exports.LaborEventsController = void 0;
const LaborEventsService_1 = require("../service/LaborEventsService");
class LaborEventsController {
    /**
     ...
     */
    static async createLaborEvent(req, res) {
        const laborData = req.body;
        try {
            const newLaborEvent = await LaborEventsService_1.LaborEventsService.createLaborEvent(laborData);
     ...
     */
    static async getAllLaborEvents(req, res) {
        try {
            // Return both event types and assignments for the calendar
            const laborEvents = await LaborEventsService_1.LaborEventsService.getAllLaborEvents();
            const employeeEvents = await LaborEventsService_1.LaborEventsService.getAllEmployeeLaborEvents();
     ...
     */
    static async updateLaborEvent(req, res) {
        const laborEventId = parseInt(req.params.id, 10);
        const laborEventData = req.body;
        try {
            const updatedLaborEvent = await LaborEventsService_1.LaborEventsService.updateLaborEvent(laborEventId, laborEventData);
     ...
     */
    static async deleteLaborEvent(req, res) {
        const laborEventId = parseInt(req.params.id, 10);
        try {
            const deleted = await LaborEventsService_1.LaborEventsService.deleteLaborEvent(laborEventId);
            if (!deleted) {
                return res.status(404).json({ error: "Labor event not found" });
            }
     ...
     */
    static async assignLaborEventsToEmployee(req, res) {
        try {
            ...
            const result = await LaborEventsService_1.LaborEventsService.assignLaborEventsToEmployee(eventData);
     ...
     */
    static async deleteEmployeeLaborEvent(req, res) {
        try {
            const id = Number(req.params.id);
            if (Number.isNaN(id)) {
                return res.status(400).json({ message: 'Invalid id parameter' });
            }
            const deleted = await LaborEventsService_1.LaborEventsService.deleteEmployeeLaborEvent(id);
            if (!deleted) {
...
exports.LaborEventsController = LaborEventsController;
