...
const PayrollService_1 = require("../service/PayrollService");
class PayrollController {
    ...
    static async getAllPayrolls(req, res) {
        try {
            const payrolls = await PayrollService_1.PayrollService.getAllPayrolls();
            res.json({
                success: true,
                data: payrolls
            });
    ...
    static async createPayroll(req, res) {
        try {
            ...
            const payroll = await PayrollService_1.PayrollService.createPayroll(payrollData);
    ...
    static async getPayrollById(req, res) {
        try {
            const payroll = await PayrollService_1.PayrollService.getPayrollById(Number(req.params.id));
            if (!payroll)
    ...
    static async updatePayroll(req, res) {
        try {
            const payroll = await PayrollService_1.PayrollService.updatePayroll(Number(req.params.id), req.body);
            if (!payroll)
                return res.status(404).json({ error: "Payroll not found" });
            res.json(payroll);
        }
     ...
     */
    static async getPayrollEmployees(req, res) {
        try {
            const employees = await PayrollService_1.PayrollService.getPayrollEmployees(Number(req.params.id));
            res.json({
                success: true,
                data: employees
            });
