...
exports.PayrollTypesController = void 0;
const PayrollTypeService_1 = require("../service/PayrollTypeService");
class PayrollTypesController {
     ...
     */
    static async createPayrollType(req, res) {
        try {
            const payrollType = await PayrollTypeService_1.PayrollTypeService.createPayrollType(req.body);
     ...
     */
    static async updatePayrollType(req, res) {
        ...
        try {
            const updatedPayrollType = await PayrollTypeService_1.PayrollTypeService.updatePayrollType(id, req.body);
     ...
     */
    static async getPayrollType(req, res) {
        const id = parseInt(req.params.id, 10);
        if (isNaN(id)) {
            res.status(400).json({ error: "Invalid payroll type ID" });
            return;
        ...
        try {
            const payrollType = await PayrollTypeService_1.PayrollTypeService.getPayrollTypeById(id);
            if (!payrollType) {
                res.status(404).json({ error: "Payroll type not found" });
                return;
            }
     ...
     */
    static async getAllPayrollTypes(req, res) {
        try {
            const payrollTypes = await PayrollTypeService_1.PayrollTypeService.getAllPayrollTypes();
...
exports.PayrollTypesController = PayrollTypesController;
