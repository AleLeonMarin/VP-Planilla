  ...
  const renderReportTarget = (target: ReportTargetSummary) => (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="font-semibold text-zinc-700 dark:text-white">{target.institution}</span>
      </div>
      <div className="text-sm text-[#4A5D3A] dark:text-zinc-400">
        <p className="truncate text-xs text-gray-500 dark:text-zinc-400">{target.endpoint_url}</p>
        <p className="text-sm text-gray-600 dark:text-zinc-400">{target.contact_email}</p>
      </div>
    </div>
  );
        ...
        <header className="rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-6 shadow-sm">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h1 className="mt-1 text-3xl font-bold text-zinc-700 dark:text-white">Envío de comprobantes</h1>
              <p className="text-sm text-[#4A5D3A] dark:text-zinc-400">
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-[#E0D6B7] dark:divide-zinc-700 text-sm text-zinc-700 dark:text-white">
                    ...
                    <tbody className="divide-y divide-[#F2E7C9] dark:divide-zinc-700">
                      {filteredEmployees.map((employee) => (
                        <tr key={employee.employeeId} className="hover:bg-[#FBF8F0] dark:hover:bg-zinc-700">
                          ...
                          <td className="px-4 py-3">
                            ...
                            <p className="text-xs text-gray-500 dark:text-zinc-400">{employee.position || 'Sin puesto'}</p>
                          </td>
                          <td className="px-4 py-3">
                            <p className="text-sm text-[#4A5D3A] dark:text-zinc-400">{employee.email || 'Sin correo'}</p>
                            <p className="text-xs text-gray-500 dark:text-zinc-500">CCSS: {employee.socialSecurityCode || 'N/A'}</p>
                          </td>
                          ...
                          <td className="px-4 py-3 text-right font-semibold text-zinc-700 dark:text-white">
                            {formatCurrency(employee.netSalary)}
                          </td>
                          <td className="px-4 py-3">
                            {employee.lastDispatch ? (
                              <div className="text-xs text-gray-600 dark:text-zinc-400">
                                <p className="font-semibold text-zinc-700 dark:text-white">{employee.lastDispatch.type}</p>
                              ...
                              <span className="text-xs text-gray-400 dark:text-zinc-500">Sin registros</span>
                            )}
                          </td>
                        </tr>
                      ))}
                      {filteredEmployees.length === 0 && (
                        <tr>
                          <td colSpan={7} className="px-4 py-8 text-center text-sm text-gray-500 dark:text-zinc-400">
          ...
          <div className="flex flex-col gap-4">
            <div className="rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-5 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-zinc-700 dark:text-white">Configuración del envío</p>
                  <p className="text-xs text-gray-500 dark:text-zinc-400">Personaliza los destinatarios y notas.</p>
                </div>
              </div>
              ...
              <div className="flex max-h-64 flex-col gap-3 overflow-y-auto pr-2">
                ...
                {logs.map((log) => (
                  <div key={log.id} className="rounded-2xl border border-[#F0E6C5] dark:border-zinc-600 bg-[#FBF8F0] dark:bg-zinc-700 p-3">
                    ...
                    <p className="text-xs text-gray-500 dark:text-zinc-400">{log.employeeName || 'Empleado'}</p>
                    <span
            ...
            {dispatchSummary && (
              <div className="rounded-3xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-5 shadow-sm">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-zinc-700 dark:text-white">Resumen del último envío</p>
                </div>
                ...
                <div className="mt-3 max-h-32 overflow-y-auto pr-1">
                  {dispatchSummary.results.map((result) => (
                    <div key={`${result.employeeId}-${result.status}-${result.detail}`} className="border-b border-[#F0E6C5] dark:border-zinc-600 py-2 last:border-0">
                      <p className="text-sm font-semibold text-zinc-700 dark:text-white">{result.employeeName}</p>
                      <p className="text-xs text-gray-500 dark:text-zinc-400">{result.detail}</p>
                      <span
