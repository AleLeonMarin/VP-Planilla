...
export default function ReportsPage() {
  ...
  const renderReportTarget = (target: ReportTargetSummary) => (
    <div
      key={target.id}
      className="flex flex-col gap-2 rounded-2xl border border-[#E0D6B7] bg-white/80 p-4"
    >
      <div className="flex items-center gap-2">
        <ShieldCheckIcon className="h-5 w-5 text-[#6F7153]" />
        <span className="font-semibold text-[#3B4D36]">{target.institution}</span>
      </div>
      <div className="text-sm text-[#4A5D3A]">
        <p className="truncate text-xs text-gray-500">{target.endpoint_url}</p>
        <p className="text-sm text-gray-600">{target.contact_email}</p>
      </div>
    </div>
  );

  return (
    <div className="h-full overflow-auto bg-[#E7DCC1] p-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <header className="rounded-3xl border border-[#E0D6B7] bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm uppercase tracking-wide text-[#6F7153]">Reportes oficiales</p>
              <h1 className="mt-1 text-3xl font-bold text-[#3B4D36]">Envío de comprobantes</h1>
              <p className="text-sm text-[#4A5D3A]">
                Genera y envía los comprobantes en formato XML para CCSS y Hacienda con un clic.
              </p>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 rounded-2xl border border-[#E0D6B7] bg-[#F9F1DC] px-4 py-2">
              ...
              <div className="flex items-center gap-2 rounded-2xl border border-[#E0D6B7] bg-[#F9F1DC] px-4 py-2">
                ...
                <span className="text-sm font-semibold text-[#3B4D36]">Generación XML</span>
              </div>
            </div>
          </div>
        </header>

        <section className="grid gap-4 rounded-3xl border border-[#E0D6B7] bg-white p-6 shadow-sm lg:grid-cols-4">
          <div className="lg:col-span-2">
            <label className="text-sm font-semibold text-[#3B4D36]">
              Planilla disponible
            </label>
            <select
              value={selectedPayrollId ?? ''}
              onChange={(event) => setSelectedPayrollId(Number(event.target.value))}
              className="mt-2 w-full rounded-2xl border border-[#E0D6B7] bg-[#F9F1DC] px-4 py-3 text-[#3B4D36] focus:border-[#6F7153] focus:outline-none"
            >
          ...
          <div className="rounded-2xl border border-[#E0D6B7] bg-[#F9F1DC] p-4">
            <p className="text-xs uppercase tracking-wide text-[#6F7153]">Monto neto</p>
            <p className="mt-1 text-2xl font-semibold text-[#3B4D36]">
              {formatCurrency(dataset?.payroll.total_net)}
            </p>
            <p className="text-xs text-gray-500">Periodo {formatDate(dataset?.payroll.period_start)} – {formatDate(dataset?.payroll.period_end)}</p>
          </div>
          <div className="rounded-2xl border border-[#E0D6B7] bg-[#F9F1DC] p-4">
                ...
                <button
                  ...
                  className={`rounded-xl border px-3 py-1 text-xs font-semibold transition ${
                    reportTypes.includes(type)
                      ...
                      : 'border-[#E0D6B7] bg-white text-[#6F7153]'
                  }`}
                >
                  {REPORT_LABELS[type]}
                </button>
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-4">
            <div className="flex flex-col gap-3 rounded-3xl border border-[#E0D6B7] bg-white p-4 shadow-sm md:flex-row md:items-center md:justify-between">
              <div className="flex items-center gap-2">
                <DocumentArrowUpIcon className="h-5 w-5 text-[#6F7153]" />
                <span className="text-sm font-semibold text-[#3B4D36]">
              ...
              <div className="flex flex-wrap gap-3">
                <input
                  ...
                  className="flex-1 rounded-2xl border border-[#E0D6B7] px-4 py-2 text-sm text-[#3B4D36] focus:border-[#6F7153] focus:outline-none"
                />
                <button
                  onClick={toggleSelectAll}
                  className="rounded-2xl border border-[#E0D6B7] px-4 py-2 text-sm font-semibold text-[#3B4D36] hover:border-[#6F7153]"
                >
            ...
            <div className="rounded-3xl border border-[#E0D6B7] bg-white shadow-sm">
              {loadingDataset ? (
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-[#E0D6B7] text-sm text-[#3B4D36]">
                    <thead className="bg-[#F9F1DC]">
                      <tr>
                    ...
                    <tbody className="divide-y divide-[#F2E7C9]">
                      {filteredEmployees.map((employee) => (
                        <tr key={employee.employeeId} className="hover:bg-[#FBF8F0]">
                          ...
                          <td className="px-4 py-3">
                            ...
                            <p className="text-xs text-gray-500">{employee.position || 'Sin puesto'}</p>
                          </td>
                          <td className="px-4 py-3">
                            <p className="text-sm text-[#4A5D3A]">{employee.email || 'Sin correo'}</p>
                            <p className="text-xs text-gray-500">CCSS: {employee.socialSecurityCode || 'N/A'}</p>
                          </td>
                          ...
                          <td className="px-4 py-3 text-right font-semibold text-[#3B4D36]">
                            {formatCurrency(employee.netSalary)}
                          </td>
                          <td className="px-4 py-3">
                            {employee.lastDispatch ? (
                              <div className="text-xs text-gray-600">
                                <p className="font-semibold text-[#3B4D36]">{employee.lastDispatch.type}</p>
          ...
          <div className="flex flex-col gap-4">
            <div className="rounded-3xl border border-[#E0D6B7] bg-white p-5 shadow-sm">
              <div className="flex items-center gap-3">
                <EnvelopeIcon className="h-6 w-6 text-[#6F7153]" />
                <div>
                  <p className="text-sm font-semibold text-[#3B4D36]">Configuración del envío</p>
                  <p className="text-xs text-gray-500">Personaliza los destinatarios y notas.</p>
                </div>
              </div>
              <div className="mt-4 space-y-3">
                <div>
                  <label className="text-xs font-semibold text-[#3B4D36]">
                    Copia (CC) opcional
                  </label>
                  <input
                    value={ccInput}
                    ...
                    className="mt-1 w-full rounded-2xl border border-[#E0D6B7] px-3 py-2 text-sm focus:border-[#6F7153] focus:outline-none"
                  />
                </div>
                <div>
                  <label className="text-xs font-semibold text-[#3B4D36]">
            ...
            <div className="rounded-3xl border border-[#E0D6B7] bg-white p-5 shadow-sm">
              <div className="mb-3 flex items-center gap-2">
                ...
                <p className="text-sm font-semibold text-[#3B4D36]">Destinos oficiales</p>
              </div>
            ...
            <div className="rounded-3xl border border-[#E0D6B7] bg-white p-5 shadow-sm">
              <div className="mb-3 flex items-center gap-2">
                ...
                <p className="text-sm font-semibold text-[#3B4D36]">Últimos envíos</p>
              </div>
              <div className="flex max-h-64 flex-col gap-3 overflow-y-auto pr-2">
                {logs.length === 0 && (
                  <p className="text-sm text-gray-500">Sin historial para esta planilla.</p>
                )}
                {logs.map((log) => (
                  <div key={log.id} className="rounded-2xl border border-[#F0E6C5] bg-[#FBF8F0] p-3">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-semibold text-[#3B4D36]">{log.type}</span>
                      <span>{formatDate(log.generated_at)}</span>
                    </div>
                    <p className="text-xs text-gray-500">{log.employeeName || 'Empleado'}</p>
                    <span
            ...
            {dispatchSummary && (
              <div className="rounded-3xl border border-[#E0D6B7] bg-white p-5 shadow-sm">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-[#3B4D36]">Resumen del último envío</p>
                </div>
                ...
                <div className="mt-3 max-h-32 overflow-y-auto pr-1">
                  {dispatchSummary.results.map((result) => (
                    <div key={`${result.employeeId}-${result.status}-${result.detail}`} className="border-b border-[#F0E6C5] py-2 last:border-0">
                      <p className="text-sm font-semibold text-[#3B4D36]">{result.employeeName}</p>
                      <p className="text-xs text-gray-500">{result.detail}</p>
                      <span
