...
import { isLeapYear } from "@/utils/year";
...
export default function PayrollCalculatePage() {
  ...
  const adjustDatesForPayrollType = (
    ...
    const parsedCurrent = currentStartDate
      ? displayDateToDate(currentStartDate)
      : null;
    ...
    if (typeName.includes("quincenal") || typeName.includes("quincena")) {
      ...
      setStartDate(formatDateDisplay(start));
      setEndDate(formatDateDisplay(end));
    } else if (typeName.includes("aguinaldo") || typeName.includes("anual")) {
      const start = new Date(today.getFullYear(), today.getMonth(), 1);
  ...
  useEffect(() => {
    ...
    if (typeName.includes("quincenal") || typeName.includes("quincena")) {
      ...
      if (isLeapYear(start)) {
        if (diffDays < 366) {
          warning = `El rango actual es de ${diffDays} días. Para calcular el aguinaldo se requieren 366 días`;
        } else if (diffDays > 366) {
          warning = `El rango actual es de ${diffDays} días. Para calcular el aguinaldo se requieren 366 días`;
        }
      } else {
        if (diffDays < 365) {
          warning = `El rango actual es de ${diffDays} días. Para calcular el aguinaldo se requieren 365 días`;
  ...
  const handleCalculate = async () => {
    if (!payrollTypeId) {
      modal.showError(
        "Tipo de planilla requerido",
        "Debes seleccionar un tipo de planilla",
      );
      return;
    }
    ...
    try {
      ...
      if (typeName.includes("aguinaldo") || typeName.includes("anual")) {
        const parsedIds = employeeIdsInput
          .split(",")
        ...
        if (parsedIds.length === 0) {
          modal.showError(
            ...
            "Agrega al menos un employeeId separado por comas para calcular aguinaldo",
          );
          return;
        }

        await calculateAguinaldo(backendStartDate, backendEndDate, parsedIds);
      } else {
      ...
      modal.showSuccess(
        "Cálculo completado",
        "Se generó el resultado del cálculo",
      );
    } catch (err: unknown) {
      modal.showError(
        "Error",
        err instanceof Error ? err.message : "Error al calcular nómina",
      );
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#E7DCC1]">
      <div className="px-8 py-6 mx-auto max-w-screen-2xl">
        {/* Header */}
        <div className="flex flex-col gap-4 mb-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-bold text-[#3B4D36] leading-none">
              Cálculo de Planilla
            </h1>
            <p className="text-sm text-[#6B5B3D] mt-2">
              Selecciona el periodo para calcular la planilla y genera el
              resultado
            </p>
          </div>
        </div>
        ...
        <div className="bg-[#F5F1E8] rounded-2xl shadow-sm border border-[#E0D6B7] p-6 mb-6">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
            ...
            {(payrollTypes
              ?.find((pt) => pt.id === payrollTypeId)
              ?.name.toLowerCase()
              .includes("aguinaldo") ??
              false) && (
              <div>
                <label className="block text-sm font-medium text-[#5D4E37] mb-2">
                  Employee IDs (coma separados){" "}
          ...
          {payrollTypeId && payrollTypes && (
            <div className="p-4 mt-5 border border-blue-200 rounded-lg bg-blue-50">
              <p className="text-sm text-blue-800">
                <span className="font-semibold">📋 Tipo seleccionado:</span>{" "}
                {
                  payrollTypes.find((pt) => pt.id === payrollTypeId)
                    ?.description
                }
              </p>
            </div>
          ...
          <div className="mt-5 p-4 bg-[#F9F3E3] rounded-lg border border-[#E7DCC1]">
            <p className="text-sm text-[#5D4E37] leading-relaxed">
              <span className="font-semibold">💡 Nota:</span> El cálculo
              incluirá todos los empleados activos en el periodo seleccionado,
              considerando salarios, bonificaciones, deducciones y horas
              trabajadas.
              {!payrollTypeId &&
                " Selecciona primero un tipo de planilla para ajustar automáticamente las fechas."}
            </p>
          </div>
        </div>
        ...
        {/* Error message */}
        {error && (
          <div className="p-4 mb-6 text-red-800 border border-red-300 rounded-lg shadow-sm bg-red-50">
