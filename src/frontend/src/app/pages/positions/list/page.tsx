  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl font-semibold text-zinc-700 dark:text-white">Posiciones</h2>
        <div>
          <button onClick={() => refetch()} className="mr-2 px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-zinc-200 rounded hover:bg-gray-300 dark:hover:bg-zinc-600">Recargar</button>
      ...
      <FormModal open={formOpen} onClose={() => setFormOpen(false)} title={editing ? 'Editar Posición' : 'Nueva Posición'} initialValues={editing || undefined} onSubmit={handleSubmit}>
        {(methods: UseFormReturn<Partial<Position>>) => (
          <div className="grid grid-cols-1 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1 text-zinc-700 dark:text-white">Nombre</label>
              <input {...methods.register('name')} className="w-full border border-gray-300 dark:border-zinc-600 bg-white dark:bg-gray-700 px-2 py-1 rounded text-zinc-700 dark:text-white" />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1 text-zinc-700 dark:text-white">Descripción</label>
              <input {...methods.register('description')} className="w-full border border-gray-300 dark:border-zinc-600 bg-white dark:bg-gray-700 px-2 py-1 rounded text-zinc-700 dark:text-white" />
            </div>

            <div>
              <label className="block text-sm font-medium mb-1 text-zinc-700 dark:text-white">Salario base</label>
              <input {...methods.register('base_salary', { valueAsNumber: true })} className="w-full border border-gray-300 dark:border-zinc-600 bg-white dark:bg-gray-700 px-2 py-1 rounded text-zinc-700 dark:text-white" />
            </div>
